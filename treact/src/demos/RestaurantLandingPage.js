...
const RecommendationCard = tw(
	Link
)`flex flex-col bg-white rounded-lg shadow-lg overflow-hidden transform hocus:scale-105 transition-transform duration-300`;
...
export default () => {
	...
	const fetchRecipesForTabs = async (currentUser) => {
		try {
			const response = await fetch(
				`http://localhost/TCC/php/get_recipes.php?userId=${currentUser.id_usuario}`
			);
	...
	function getPeriodo(asString = false) {
		...
		if (hora >= 4 && hora < 10)
			return asString ? "Café da manhã" : <CafeText>café da manhã?</CafeText>;
		if (hora >= 10 && hora < 14)
			return asString ? "Almoço" : <AlmocoText>almoço?</AlmocoText>;
		if (hora >= 14 && hora < 17)
			return asString ? (
				"Lanche da tarde"
			) : (
				<LancheText>lanche da tarde?</LancheText>
			);
	...
	return (
		<AnimationRevealPage disabled>
			...
			{user && <WelcomeMessage>Bem vindo, {user.nome_usuario}!</WelcomeMessage>}
			<FormContainer
				onSubmit={async (e) => {
					e.preventDefault(); // Aqui você pode filtrar dados, chamar API, etc.
					...
					try {
						...
						const response = await fetch(
							"http://localhost/TCC/php/processo.php",
							{
								method: "POST",
								headers: {
						...
						try {
							...
							if (data.success) {
								setRecipeResult({
									name: data.elements_used,
									instructions: data.receita,
								});
							...
							setError(
								"Erro de formato na resposta do servidor (SyntaxError). Veja o console."
							);
						}
					} catch (error) {
			...
			{isLoading && (
				<ResultContainer>
					<ResultText>Gerando receita, por favor aguarde...</ResultText>
				</ResultContainer>
			)}
			{error && (
				<ResultContainer>
				...
				<RecommendationsContainer>
					...
					<RecommendationsGrid>
						{recommendations.map((recipe) => (
							<RecommendationCard
								key={recipe.id_receita}
								to={`/receita/${recipe.id_receita}`}>
								...
								<RecommendationContent>
									...
									<RecommendationMatch>
										Você tem {recipe.ingredientes_em_comum} de{" "}
										{recipe.total_ingredientes} ingredientes!
									</RecommendationMatch>
								</RecommendationContent>
							</RecommendationCard>
						))}
